...
# gridTS
for f in bdy_NATL60-GLORYS2_[ns]*_TS_*.nc ; do
   ...
   ncrename -h -d record,yt -d x,xt  ${f}.ok
   rm f[1234]*.nc
done

# gridU
for f in bdy_NATL60-GLORYS2_[ns]*_U_*.nc ; do
   ncks -h -O --fix_rec_dmn time_counter $f f0.nc
   ncecat -h f0.nc f1.nc
   ncks -h -O --fix_rec_dmn record  f1.nc f2.nc
...
# gridV
for f in bdy_NATL60-GLORYS2_[ns]*_V_*.nc ; do
   ncks -h -O --fix_rec_dmn time_counter $f f0.nc
   ncecat -h f0.nc f1.nc
   ncks -h -O --fix_rec_dmn  record f1.nc f2.nc
