...
submit() { cd ${P_CTL_DIR}; pwd; oarsub -S  $1 > $TMPDIR/logsubmit ;}
# ---

# function for running OPA : it takes the number of procs and name of program as argument
runcode() {
         mpiexec -machinefile $OAR_NODE_FILE -ib -envall -n $*
          }
# ---
...
save_arch_file () {
      echo nothing to do on froggy ! > /dev/null
                  }
# ---
...
mk_batch_hdr() {
   ...
   # build FROGGY batch header
   cat << eof 
#!/bin/bash
#OAR -n $name
#OAR --project $account
#OAR -l /nodes=$nodes,walltime=$wallclock
eof
               }
######################################################################################
