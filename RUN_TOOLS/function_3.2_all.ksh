#!/bin/bash
# $Id: function_3.2_all.ksh 1750 2017-02-17 16:02:51Z molines $
######################################################################################
...
geticeinit()        {
     ...
     if [ $tmp = T ] ; then
       SetYears
       filter=''  # take all sn_xxx of the block
       blk=namiceini ;  getfiles  $blk $P_DTA_DIR $F_DTA_DIR namelist_ice
                         getweight $blk $P_DTA_DIR $F_DTA_DIR namelist_ice
...
eof1
   cat ztmp | sed -e 's/@/\$/g' > ./$1\${mmm}.ksh    # change @ into \$ and create script for current member
   chmod 755 ./$1\${mmm}.ksh                         # made it executable

   mpmd_arg="\$mpmd_arg 1 ./$1\${mmm}.ksh"           # prepare the command line for runcode function
 done
  
  pwd
  if [ ! \$mmm ] ; then
     ./$1\${mmm}.ksh                                 # not an ensemble run : serial process
  else
...
eof1
   cat zbnce | sed -e 's/@/\$/g' > ./$1\${mmm}.ksh    # change @ into \$ and create script for current member
   chmod 755 ./$1\${mmm}.ksh                         # made it executabl  
   mpmd_arg="\$mpmd_arg 1 ./$1\${mmm}.ksh"           # prepare the command line for runcode function
...
eof1
   cat zbxiosens | sed -e 's/@/\$/g' > ./$1\${mmm}.ksh
   chmod 755 ./$1\${mmm}.ksh                         # made it executabl  
   mpmd_arg="\$mpmd_arg 1 ./$1\${mmm}.ksh"           # prepare the command line for runcode function
...
# Make batch header for submitted scripts ( common part to all machines). Called by specific machine function in function_3.2_machine.ksh (mk_batch_hdr) with same arguments
