...
rcopy $P_CTL_DIR/includefile.ksh_${JPNI}_${JPNJ}_${JPNIJ} includefile.ksh 
...
sed -e "s/NUMERO_DE_RUN/$no/" \
    ...
    -e "s@CN_DIROUT@$DDIR/${CONFIG_CASE}-${DIROUText}_${JPNI}_${JPNJ}_${JPNIJ}.$no@" \
...
chkdir  $DDIR/${CONFIG_CASE}-${DIROUText}_${JPNI}_${JPNJ}_${JPNIJ}.$no
...
eof
    ...
    cat iodef.xml | sed -e "s@<OUTDIR>@$DDIR/${CONFIG_CASE}-XIOS_${JPNI}_${JPNJ}_${JPNIJ}.$no@"  \
        -e "s@<MOORDIR>@$DDIR/${CONFIG_CASE}-MOORINGS.$no@" \
        -e "s/<CONFIG>/$CONFIG/" -e "s/<CASE>/$CASE/" \
        -e "s/<NDATE0>/$ndate0/" > ztmp
    mv ztmp iodef.xml
    if [ $XIOS2 = 1 ] ; then
       cat file_def.xml | sed -e "s@<OUTDIR>@$DDIR/${CONFIG_CASE}-XIOS_${JPNI}_${JPNJ}_${JPNIJ}.$no@"  \
        -e "s@<MOORDIR>@$DDIR/${CONFIG_CASE}-MOORINGS.$no@" \
        -e "s/<CONFIG>/$CONFIG/" -e "s/<CASE>/$CASE/" \
        -e "s/<NDATE0>/$ndate0/" > ztmp
