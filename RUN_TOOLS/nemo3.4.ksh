# $Id: nemo3.4.ksh 366 2012-01-11 14:25:17Z molines $
...
ICE=0     ;  if [ $(keychk key_lim    )  ] ; then ICE=1     ; fi
...
# Ice model
ICE_INI=0 ; ICE_INI4=0; ICE_DMP=0; ICE_INI_FILE=0
if [ $ICE = 1 ] ; then
    tmp=$(LookInNamelist ln_limini namelist_ice) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then ICE_INI=1 ; fi
    tmp=$(LookInNamelist ln_iceini namelist_ice) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then ICE_INI4=1 ; fi
    tmp=$(LookInNamelist ln_iceini_file namelist_ice) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then ICE_INI_FILE=1 ; fi
    tmp=$(LookInNamelist ln_limdmp namelist_ice) ; tmp=$(normalize $tmp)
    if [ $tmp = T ] ; then ICE_DMP=1 ; fi
fi
echo "   ***  ICE = $ICE"
echo "   ***  ICE_INI = $ICE_INI"
echo "   ***  ICE_INI4 = $ICE_INI4"
echo "   ***  ICE_INI_FILE = $ICE_INI_FILE"
...
if [ $ICE_INI = 1 -a $no -eq 1 -a $ICE_INI4 = 0 ] ; then
    rapatrie $ICEINI  $P_I_DIR $F_INI_DIR $OPA_ICEINI
fi

if [ $ICE_INI4 = 1 -a $no -eq 1 -a $ICE_INI_FILE = 1 ] ; then
    geticeinit
fi
    ...
    if [ $DIMGOUT = 1 ] ; then
      ...
      # extra non standard output  {
        if [ $nwri2 != 0 ] ; then
            ...
            mkbuild_ssf zrebssf.$ext.ksh
            submit ${P_CTL_DIR}/zrebssf.$ext.ksh
        fi

        if [ $nwri3 != 0 ] ; then
            echo '   ***  process the rebuild of dimgproc files (5D)'
            mkbuild_5d  zreb5d.$ext.ksh
            submit ${P_CTL_DIR}/zreb5d.$ext.ksh
        fi
      #                           }
        ...
        if [ $AGRIF = 1 ] ; then
            for idx in ${agrif_pref[@]} ; do
                ...
                mkbuildnc zrebag${idx}.$ext.ksh $idx
                submit ${P_CTL_DIR}/zrebag${idx}.$ext.ksh
            done
        fi
    fi   #  
...
#      # look for zoom (in one_file mode). Assume domain_ref without 'grid' keyword
       ...
       # check enabled zoom
        ...
        if [ $zoom_enable ] ; then
          if [ ! -f $CTL_DIR/post_process_zoom.ksh ] ; then
             mk_post_process post_process_zoom.ksh
             cp post_process_zoom.ksh $CTL_DIR
           else
             cp $CTL_DIR/post_process_zoom.ksh ./
          fi
...
# JMM CAUTION not a generic function for all machines ...
            ...
            for n in $(seq $ENSEMBLE_START $ENSEMBLE_END ) ; do
              cmd="$cmd -np 1 ./post_process_zoom.ksh $n $ext :"
            done
            cmd=${cmd%:}
            $cmd
        #   runcode_mpmd ./zpp.conf.$ext
          else
             ./post_process_zoom.ksh -1 $ext
          fi
          cd $TMPDIR
        fi
        ...
        if [ $MULTIPLE_FILE = 1 ] ; then
            if [ $MERGE = 0 ] ; then
                ...
                mkbuild_merge zmergxios.$ext.ksh  
                submit ${P_CTL_DIR}/zmergxios.$ext.ksh
              ...
              # 
                if [ $AGRIF = 1 ] ; then
                    for idx in ${agrif_pref[@]} ; do
                        ...
                        if [ $nmshag = 6 ] ; then
                            mkbuild_mesh_mask zmkmeshmask${idx}.ksh $idx  # avoid _ in name as PBS does not like !
                            submit ${P_CTL_DIR}/zmkmeshmask${idx}.ksh 
                        fi
                    done
                fi  ;;
                ( * )
...
#                tar cf $P_S_DIR/OBS/${CONFIG_CASE}${mmm}_enact_fdbk.$ext.tar enact${mmm}_fdbk_*.nc
             cmd="$cmd -np 1 ./ztarobs.$ext.ksh $mmm :"
            done
...
#             tar cf $P_S_DIR/OBS/${CONFIG_CASE}${mmm}_sla_fdbk.$ext.tar sla${mmm}_fdbk_*.nc
             cmd="$cmd -np 1 ./ztarobssla.$ext.ksh $mmm :"
            done
            cmd=${cmd%:}  # eliminate last :
            $cmd
        fi
