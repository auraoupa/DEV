   ...
   DO jj = 1, npy
    DO jk = 1, npz
     DO jt = 1, npt
        ...
        ENDIF
!        IF ( mask(ji,jj,jk,1) == 1. .AND.  var3d(ji,jj,jk,jt) == 99999. ) THEN ;  PRINT *,' mask = 1 et var = 9999 ji =',ji,' jj =',jj,' jk =',jk,' jt=',jt
        IF ( mask(ji,jj,jk,1) == 1. .AND.  var3d(ji,jj,jk,jt) == 99999. ) THEN 
          mean=0.; num=0.
          IF ( var3d(ji-1,jj  ,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji-1,jj  ,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji-1,jj+1,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji-1,jj+1,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji-1,jj-1,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji-1,jj-1,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji  ,jj+1,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji  ,jj+1,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji  ,jj-1,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji  ,jj-1,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji+1,jj  ,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji+1,jj  ,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji+1,jj-1,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji+1,jj-1,jk,jt); num=num+1.; ENDIF
          IF ( var3d(ji+1,jj+1,jk,jt) < 1000. ) THEN; mean=mean+var3d(ji+1,jj+1,jk,jt); num=num+1.; ENDIF
          IF ( num == 0. ) THEN
            ...
            IF ( var3d(ji,jj,jk-1,jt) == 99999. ) THEN
              PRINT *, 'au-dessus aussi cest pourri'
            ELSE
              var3d(ji,jj,jk,jt) = var3d(ji,jj,jk-1,jt)
            ENDIF
          ELSE
            var3d(ji,jj,jk,jt)=mean/num; PRINT *,'mean = ',mean 
          ENDIF
        ENDIF
     END DO
    END DO
